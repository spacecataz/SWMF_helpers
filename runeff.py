...
PrevTime = cpu_t[PrevLoc][-1] - cpu_t[PrevLoc][0]
MedInst = np.median(eff_inst[PrevLoc[1:]])
Minutes = round(PrevTime / 60.)
EffHour = (run_t[PrevLoc][-1] - run_t[PrevLoc][0]) / PrevTime
...
line = f"Total Efficiency over past {Minutes} mins = {EffHour:06.3f}X"
