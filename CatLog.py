...
Examples:
    ...
    3) Combine all geoindex files into a new, single file:
       > CatLog.py -o=geoindex_all.log geoindex*.log
'''
...
# Set raw input command based on python version:
...
if args.debug:
    ...
    for f in args.files:
        print("\t{}\n".format(f))
    if rawinput('Is this correct? [y/N]') != 'y': 
    ...
    for i, part in enumerate(head.split()):
        for s in iter_names:
            if s == part.lower():
                ...
                time_vars.append(s)
                break
        if time_locs: break # Only want a single iteration tag.
...
#out.seek(-1*nbytes, 2) #seek last line. THIS USED TO WORK IN PYTHON2.
...
for f in args.files:
    ...
    if args.debug: print("Processing {}:".format(f))
    nextfile=open(f, 'r')
    ...
    if not args.nocheck:
        ...
        while nSkip<nLines:
            if get_time(lines[0], time_locs) > lasttime:
                ...
                lines.pop(0)
                nSkip+=1
        if args.debug: print("\tFound {} overlapping lines.".format(nSkip))
    # Append data to first log.
    if len(lines)<1: continue
    for l in lines:
        out.write(l)
    # Save "last time".
    if not args.nocheck: lasttime=get_time(lines[-1], time_locs)
    # Delete file when done.
    if args.remove: 
        if args.debug: print("\tRemoving file.")
        unlink(f)
