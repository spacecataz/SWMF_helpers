...
parser.add_argument("-m", "--mag", help="Add magnetic field lines to y=0 slice",
                    action="store_true")
...
parser.add_argument("-n", "--nthread", type=int, default=1,
                    help="If >1, plots will be made in parallel using NTHREADS"+
                    " threads")
...
parser.add_argument("-s", "--sats", action="store_true", help="Add virtual "+
                    "satellite positions and names to plot using all sat*.sat"+
                    " files found")
...
parser.add_argument("-v2", "--var2", default='rho', help="Set the variable to "+
                    "plot in the Z=0 frame.  Defaults to 'rho' for total mass"+
                    " density")
parser.add_argument("-c1", "--cmap1", default='plasma', help='Set the Matplotlib '+
                    'color map for the Y=0 frame.  Any MPL table name can be '+
                    'used; defaults to "plasma"')
...
parser.add_argument('-start', default=None, help="Set the start date and time "+
                    "of the simulation using the format YYYY-MM-DDTHH:MN:SS. "+
...
def plot_results(fileY, fileZ, iFile=0, nFiles=1):
    ...
    a2.set_title('{} ({})'.format(pb.mhdname_to_tex(v2), mZ[v2].attrs['units']))
    stuff[-1].set_label('')
